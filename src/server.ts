...
/**
 * Fill in versions not found in packagesDir with empty array (could arguably
 * flag an error)
 */
...
function getPackagesDirectoryContents() {
	...
	let dirs: string[] = readdirSync(config.packagesDir).filter(function (
		file: string
	) {
...
/**
 * See if any versions are present in the config file (they're not supposed to
 * be!)
 */
...
async function readConfig() {
	...
	// Allow the elements property to be an object, where the key is the `abbr`
	// name
...
if (loggerDetails.maxFiles === true) {
	console.log(
		'\x1b[31mERROR:\x1b[37m maxFiles option set to true, requires a number of files to be specified. Ending.'
	);
	...
	console.log(
		'\x1b[31mERROR:\x1b[37m frequency option set to true, requires a frequency to be specified. Ending.'
	);
...
// If there are more options defined which are not defined then print the help
// and end server
if (Object.keys(argum).length > 37) {
	// Not sure of the value of this test but leaving in in case I'm missing
	// something (COLIN)
	console.log(
		'\x1b[31mERROR:\x1b[37m Unexpected command-line option. Ending.'
	);
	logger.help();
	...
	console.log(
		'\x1b[31mERROR:\x1b[37m Logfile option set to true but no file location specified. Ending.'
	);
	logger.help();
	...
	console.log(
		'\x1b[31mERROR:\x1b[37m ErrorLogFile option set to true but no file location specified. Ending.'
	);
	logger.help();
	...
	console.log(
		'\x1b[31mERROR:\x1b[37m AccessLogFile option set to true but no file location specified. Ending.'
	);
	logger.help();
	...
	console.log(
		'\x1b[31mERROR:\x1b[37m port option set to true but no port specified. Ending.'
	);
	logger.help();
...
/*
 * This is the server for requesting files to be built. It validates the URL
 * that it takes and builds the file before returning them to the user.
 */
let server = http
	.createServer(async function (req, res) {
		...
		// If a signal is received to re-read the config file then wait until
		// the next request before reading it (to save signal handler time)
		if (getConfig) {
			...
			// DD-1267 - the value in the config isn't being reread when server
			// is signalled.
		...
		if (
			...
			logger.error(
				id +
					' - 405 Bad Request. Types not permitted "POST", "PUT" and "DELETE". Type method attempted ' +
					req.method
			);
		}
		// If the URL has a latest parameter run the following to find the
		// latest version of the listed modules
		else if (
			splitURL[0] === '/latest' &&
			splitURL.length === 1 &&
			!config.latestAll
			...
			logger.error(
				id +
					' - 404 Invalid URL. latestAll not enabled but was requested.'
			);
		}
			...
			// Run validateLatest to validate the URL is legal and return the
			// full URL required by the user but don't bother validating if user
			// wants all versions (since some are incompatible)
			...
			if (!latest) {
				...
				logger.error(
					id +
						' - 404 Invalid URL. Failed to find latest version(s) of the module(s) requested. ' +
						req.url
				);
			}
			else {
				...
				// For every filetype build the file and push to the array for
				// hashing purposes
				for (let extension of config.fileExtensions) {
					let contentAddition = await bui.buildFile(
						latest + config.fileNames[0] + extension
					);

					if (!contentAddition) {
						res.writeHead(500);
						logger.error(
							id + ' - 500 Internal Server error. ' + req.url
						);
						res.end();
						return;
					}
			...
			if (config.selectHack) {
				let parsedURL = splitURL[0].split(
					new RegExp('[' + config.separators.join('') + ']')
				);
			...
			if (content === false) {
				res.writeHead(500);
				logger.error(
					id + ' - 500 File not found when building file ' + req.url
				);
			}
			else if (content === 404) {
				res.writeHead(404);
				logger.error(
					id + ' - 404 File not found when building file ' + req.url
				);
			}
			else if (content === 400) {
				res.writeHead(400);
				logger.error(
					id + ' - 400 Bad request when building file ' + req.url
				);
			}
			else if (
				splitURL.indexOf('details') === 1 &&
				...
				res.write(
					JSON.stringify(
						meta.getDetails(content, await bui.getInclusions(), t0)
					)
				);
				...
				// Find the file type that has been requested and the content
				// type for http
...
// Function to cause the config to be re read when a SIGUSR1 signal is received.
process.on('SIGUSR1', () => {
	getConfig = true;
