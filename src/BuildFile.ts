...
/**
 * This interface defines the format of the object to hold the details of all of
 *   the individual elements required to build the file.
 */
interface IDetails {
	version: string;
	folderName: string;
...
/**
 * This interface defines the format of the Includes object used throughout the
 * BuildFile Class.
 */
interface IIncludes {
	[key: string]: string;
...
export default class BuildFile {
	 ...
	 *
	 * @param cache The current Cache of most recent files to have been
	 * requested
	 * @param configIn The configuration of the CDN Server. This includes the
	 *   build location of the files and all of their details.
	 * @param debug A boolean value to defined whether the Server is being run
	 * in debug mode.
	 */
	constructor(
		cache: Cache,
	...
	/**
	 * Fetches all of the sub files and converges them all into one final file
	 * to be returned to the user
	 *
	 * @param filePath The path of the file specified. Each element specifies a
	 *   module to be included.
	 * @returns {boolean | string} Returns the file content if it is built
	 *   successfully and a boolean value to indicate a failure.
	 */
	public async buildFile(
		...
		// If the parsedURL's first element is and empty string then it begins
		// with a separator, which is legal but will cause errors further on
		// unless we remove the empty string from the array. If the URL
		// Requested contains multiple separators at the start of the URL then
		// this will be picked up in the Validation stage.
		if (parsedURL[0] === '') {
		...
		// If an static file is requested then there is a need to return only
		// that file so do the following and return
		if (extraIndex !== -1) {
			...
			this._logger.debug(
				this._id + ' - Checking for ' + extras + ' in cache'
			);
			let fromCache = this._storedFiles.searchCache(path, this._id);

			if (!fromCache) {
				let content = await fileExists(path);
				return this._logUpdateReturn(
					extras + ' not found in cache, fetching Image from ' + path,
					path,
					content,
					false
				);
		...
		// ReOrder the URL so that the files are added together in the correct
		// order as defined in the config
		parsedURL = this._reOrderBuild(parsedURL);

		// Grab the Folder names, File names, versions, orders and includes and
		// add them to a list.
		...
		// From the file name, identify filetype, and whether the minified
		// version is to be built
		let splitFileName: string[] =
			parsedURL[parsedURL.length - 1].split('.');
		...
		if (!file) {
			...
			// Find all of the matching strings in the file for the RegExp as
			//  given Then replace them with empty strings as we do not want to
			//  include them
			let sourceMapReg = new RegExp(
				'\\/\\*# sourceMappingURL=.*\\*\\/',
				'g'
			);
			...
			// Find all of the matching strings for source maps in js as given
			// by the below regex, then replace with empty strings
	...
	/**
	 * This function coordinates the building of all of the files, it fetches
	 * each sub-file and adds it to the final file to be returned to the user.
	 *
	 ...
	 * @param parsedDetails An array of the Details interface for returning the
	 * details of a build
	 * @param includesList The list of files that are to be included in the
	 * build
	 */
	private async _build(
		...
		for (let i = 0; i < parsedURL.length - 1; i++) {
			...
			if (
				parsedDetails[i].fileNameMap !== undefined &&
				parsedDetails[i].fileNameMap.get(folder) !== undefined
			) {
				...
				for (let name of fileNameArray) {
					...
					for (
						let includesPoint = 0;
						includesPoint < includesKeys.length;
						includesPoint++
					) {
						updatestring = updatestring.replace(
							'{' + includesKeys[includesPoint] + '}',
							includesValues[includesPoint]
						);
					}
				...
				for (let filename of updateFile) {
					...
					let fileAddition = await this._fetchReplace(
						parsedDetails[i],
						filename,
						parsedURL
					);
	...
	private async _fetchFile(
		...
		// Try to find the file and return it, if it's not found then return an
		// empty string, If an error occurs return '500' and log it.
		...
		try {
			...
			if (optional && typeof fromCache === 'string' && fromCache === '') {
				return this._logUpdateReturn(
					'Found empty optional file in cache: ' + filename,
					filename,
					'',
					true
				);
				...
				this._includedFiles.push(
					filename.replace(this._config.packagesDir, '')
				);
				return this._logUpdateReturn(
					'File found in cache: ' + filename,
					filename,
					fromCache,
					true
				);
				...
				return this._logUpdateReturn(
					'File not in Cache, but found in directory: ' + filename,
					filename,
					content,
					false
				);
				...
				return this._logUpdateReturn(
					'File not found in cache or directory',
					filename,
					'',
					false
				);
			...
			if (optional) {
				this._logger.warn(
					this._id +
						' - Unable to fetch optional file, may not exist: ' +
						filename
				);
				...
				this._logger.error(
					this._id +
						' - Unable to fetch non optional file: ' +
						filename
				);
				return false;
			}
	...
	private async _fetchReplace(parsedDetail, filename, parsedURL) {
		...
		let fileAddition = await this._fetchFile(
			path,
			parsedDetail.folderName,
			parsedDetail.version
		);
		...
		if (
			...
			if (matches !== null) {
				this._logger.debug(
					this._id + ' - Number of files found: ' + matches.length
				);
				let usefulURL = parsedURL.slice();
				usefulURL.pop();

				// For every match replace the string currently in the file with
				//  a new one that this CDN will understand when it receives a
				//  request.
				for (let match of matches) {
					let original = match.match(
						/url\s*\(\s*([\'"]?)(.*?)[\'"]?\s*\)/
					);
					...
					if (
						...
						let pathAddition = original[2]
							.split(
								new RegExp(
									'[' + this._config.separators.join('') + ']'
								)
							)
							.join('/');
		...
		// Need to also replace any BOMs that are present in the file Most JS
		// engines use utf-16 rather than utf-8, hence the utf-16 replacement
	...
	/**
	 * The URL elements come in in a different order to that which they are to
	 * be built. This function reorders them so that they are corrected.
	 *
